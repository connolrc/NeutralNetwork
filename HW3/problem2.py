...
class NeuralNetwork: 
    ...
    def adjustWeights(self, q): 
        ...
        for i in range(self.L): 
            ...
            delta_qi[i] = (self.x[q][i] - self.y[q][i]) * fprime(self.si[q][i])
            ...
            for j in range(self.M): 
                ...
                self.wdelta[1][i][j] = self.eta * delta_qi[i] * self.h[q][j] + (self.lagrange * self.w[1][i][j])
            ...
            self.w0delta[1][i] = self.eta * delta_qi[i] + (self.lagrange * self.w0[1][i])
        ...
        for j in range(self.M): 
            ...
            if q < 3: 
                rho_N = 1
                self.rho += [(1 / rho_N) * sum([self.h[q_rand][j] for q_rand in range(q) if len(self.h[q_rand]) > 1])]
            else: 
                rho_N = random.randint(1, math.ceil(q / 2))
                self.rho += [(1 / rho_N) * sum([self.h[q_rand][j] 
                                        for q_rand in np.random.choice([qq for qq in range(q)], size=rho_N, replace=False)
                                        if len(self.h[q_rand]) > 1])]
            # sort of, at least... it's taking its place mathematically
            delta_qj[j] = sum([self.w[1][i][j] * delta_qi[i] for i in range(self.L)]) 
                           
            if self.rho[j] != 0.0: 
                delta_qj[j] -= ((self.gamma * (((1 - self.rho_targ) / (1 - self.rho[j])) - (self.rho_targ / self.rho[j]))) 
                           * fprime(self.sj[q][j]))
            ...
            for k in range(self.N): 
                ...
                # calculate wdelta(t) (add the weight penalty for autoencoding regularization)
                self.wdelta[0][j][k] = self.eta * delta_qj[j] * self.x[q][k] + (self.lagrange * self.w[0][j][k])
                # update the weight with wdelta + momentum 
                self.w[0][j][k] += self.wdelta[0][j][k] + momentum_j

            # update bias weight w0
            momentum0_j = self.alpha * self.w0delta[0][j]
            self.w0delta[0][j] = self.eta * delta_qj[j] + (self.lagrange * self.w0[0][j])
    ...
    def run(self, x, ytrue, training = False, epochs = 125, eta = .001, alpha = .3, 
            op_H = .75, op_L = .25, stoch = True, gamma = 1, rho_target = .01, lagrange = .001): 
        self.x = [normalize(image) for image in x]
        self.ytrue = ytrue
        self.eta = eta
        self.alpha = alpha
        self.op_H = op_H
        ...
        for epoch in range(epochs): 
            ...
            for q in range(self.Q): 
                ...
                if sto_ran <= .5 or stoch == False: 
                    ...
                    # calculate J2
                    self.Jq[epoch] += [sum(math.pow(self.x[q][ii] - self.y[q][ii], 2) 
                                     for ii in range(self.L)) * .5]
                    
                    self.Jnum[self.ytrue[q]] += [sum(math.pow(self.x[q][ii] - self.y[q][ii], 2) 
                                     for ii in range(self.L)) * .5]
                ...
                if DEBUG_FLAG == True and sto_ran <= .05:
                    # print(".", end="")
                    print()
            ...
            if DEBUG_FLAG == True and epoch % 1 == 0: # and epoch > 0: 
                ...
                print("  Jq = {:.6f}  ".center(106, '*').format(self.getJq()))
    ...
    def getJq(self):
        return np.average(self.Jq[int(len(self.Jq)) - 1]) / 784
    ...
    # creates bar representing loss function / performance to plot in a figure
    # RETURN OUTPUTS
    # databar : plt.bar() containing the data
    # def totalLossBar(self, training = True, barColor = ""): 
    #     if training == True: 
    #         return plt.bar([1], [np.average(self.Jq[int(len(self.Jq)) - 1])], 
    #             width = .5, label = "Training Set", color = barColor)
    #     else: 
    #         return plt.bar([1.5], [np.average(self.Jq[int(len(self.Jq)) - 1])], 
    #             width = .5, label = "Test Set", color = barColor)
    ...
    def plotSampleOutputs(self, origColor = "winter", outputColor = "winter"): 
    ...
    def plotStats(self): 
        plt.figure()
        # plt.subplot(2, 1, 1)
        plt.plot([x for x in range(1, len(self.Jq) + 1)], [np.average(sublist) / 784 for sublist in self.Jq], 
...
Cortana.run(trainset, trainset_labels, training=True, stoch=True, 
            alpha=.6, eta=.001, rho_target=.04, gamma=1, lagrange=.001)
Jq_train = Cortana.getJq()
Jnum_train = Cortana.Jnum.copy()
...
writeListToFile("HW4/trainset2_output.txt", yyy)
writeListToFile("HW4/trainset2_weights_jk.txt", Cortana.w[0])
writeListToFile("HW4/trainset2_weights_ij.txt", Cortana.w[1])
writeListToFile("HW4/trainset2_bias_jk.txt", Cortana.w0[0])
writeListToFile("HW4/trainset2_bias_ij.txt", Cortana.w0[1])
writeListToFile("HW4/trainset2_Jq.txt", Cortana.Jq)
