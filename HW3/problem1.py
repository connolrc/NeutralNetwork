...
# used to normalize a vector of values between 0 and 1
normalize = lambda lst : [(el - min(lst)) / (max(lst) * 1.001 - min(lst)) for el in lst]
...
class NeuralNetwork: 
    ...
    def preloadWeights(self, file_wjk, file_wij, file_w0jk, file_w0ij): 
    ...
    def run(self, x, ytrue, training = False, epochs = 125, eta = .001, alpha = .3, 
        ...
        for epoch in range(epochs): 
            ...
            if DEBUG_FLAG == True and epoch % 1 == 0: # and epoch > 0: 
                ...
                #print("  Jq = {:.6f}      J = {:.6f}  ".center(110, '*').format(self.Jq[int(len(self.Jq)) - 1], sum(self.Jq) / len(self.Jq)))
    ...
    # # correct meaning that it was classified correctly, not necessarily 
    # # that it is what digit the datapoint image depicts. 
    # def isCorrect(self, q, i): 
    #     # if y[q][i] is the desired output neuron to be 1, according to image...
    #     if self.ytrue[q] == i: 
    #         # ... and if it was correctly identified and made the max...
    #         if self.y[q][i] == max(self.y[q]): 
    #             return True
    #         # otherwise...
    #         else: 
    #             return False
    #     # if y[q][i] does not represent the number written in the image...
    #     else: 
    #         # ... and the image was not classified as y[q][i]'s number...
    #         if self.y[q][i] != max(self.y[q]): 
    #             return True
    #         # otherwise...
    #         else: 
...
# # Cortana.run(trainset, trainset_labels, training=True, stoch=True, alpha=.7)

# # writeListToFile("HW3/trainset_output.txt", Cortana.y)
# # writeListToFile("HW3/trainset_weights_jk.txt", Cortana.w[0])
# # writeListToFile("HW3/trainset_weights_ij.txt", Cortana.w[1])
# # writeListToFile("HW3/trainset_bias_jk.txt", Cortana.w0[0])
# # writeListToFile("HW3/trainset_bias_ij.txt", Cortana.w0[1])
...
trainset_output = convertFileToList2D("HW3/trainset_output.txt")
trainset_err_frac = []
trainset_errs = []
...
testset_output = convertFileToList2D("HW3/testset_output.txt")
testset_err_frac = []
testset_errs = []
